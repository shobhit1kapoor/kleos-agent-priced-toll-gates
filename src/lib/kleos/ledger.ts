...
export function getLedgerSnapshot() {
  ...
  return {
    metrics: {
      ...
      agentSpendPermits: store.agentSpendPermits.length,
      activeSpendPermits: store.agentSpendPermits.filter((permit) => permit.status === "active").length,
      autonomousVolumeBatches: store.autonomousVolumeRuns.length,
      autonomousVolumeRuns: store.autonomousVolumeRuns.reduce((sum, run) => sum + run.completedRuns, 0),
      autonomousVolumeUsdc: Number(
        store.autonomousVolumeRuns.reduce((sum, run) => sum + run.totalUsdcMoved, 0).toFixed(6),
      ),
    ...
    agentSpendPermits: store.agentSpendPermits,
    autonomousVolumeRuns: store.autonomousVolumeRuns,
