...
export async function buildPublicStatus() {
  ...
  return {
    ...
    publicProofs: {
      ...
      agentSpendPermits: `${APP_URL}/api/agents/spend-permits`,
...
export function buildOpenApiDocument(origin = APP_URL) {
  return {
    ...
    paths: {
      ...
      "/api/agents/spend-permits": { get: { summary: "List and verify bounded agent spend permits." }, post: { summary: "Issue a budget-capped agent spend permit." } },
