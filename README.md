...
- **Traction (30%):** the app records buyer-agent runs, paid accesses, USDC
  ...
  registered/RSS-imported sources, A2A proof events, and payout splits. For final
  submission, use the live deployment with several external testers so the
  traction numbers are not only local.
...
- **Innovation (20%):** Kleos combines two-stage creator citation tolls,
  answer-linked receipt hashes, claim-level proof traces, signed creator
  webhooks, creator cash-out aggregation, tester attestations,
  value-of-information pricing, autonomous buyer budgets, MCP discovery,
  publisher manifests, retroactive impact rewards, bonded broker proof, and
...
7. POST `/api/webhooks/dispatch` and `/api/creators/cashout` to show signed
   creator notifications and Arc-ready creator cash-outs.
8. Click **Attest** or POST `/api/traction/attest` to mint a tester proof hash.
9. Inspect paid reads, bought-but-not-cited sources, answer-linked receipts,
   x402 settlement records, and collaborator split payouts.
10. Click **Reprice sources** to run the citation-aware seller pricing agent.
11. Open `/api/proof-pack` and `/api/submission/report` for the structured
   submission summary.
